...
import { AMENITY_ICONS, MAP_URL } from '@/lib/constants'
import OptimizedImage from '@/components/OptimizedImage'
import ImageGalleryModal from '@/components/ImageGalleryModal'

const FALLBACK_IMAGE = 'https://images.unsplash.com/photo-1613490493576-7fde63acd811?w=1200&q=75'
...
export default function PropertyDetail({
  ...
  const imgs = p.images?.length ? p.images : [FALLBACK_IMAGE]
  const extraCount = Math.max(0, imgs.length - VISIBLE_THUMBS)

  return (
    <article className="detail-page">
      <div className="detail-gallery-wrap">
        <button type="button" className="detail-gallery" onClick={() => setGalleryOpen(true)} aria-label="Voir toutes les photos">
          <OptimizedImage src={imgs[imgIdx]} alt={p.title} className="detail-hero-img" width={900} priority={imgIdx === 0} />
          {p.verified && (
        ...
        {imgs.length > 1 && (
          <>
            <button type="button" className="detail-nav-zone detail-nav-left" aria-label="Photo précédente" onClick={() => setImgIdx(i => Math.max(0, i - 1))} />
            <button type="button" className="detail-nav-zone detail-nav-right" aria-label="Photo suivante" onClick={() => setImgIdx(i => Math.min(imgs.length - 1, i + 1))} />
          </>
        )}
      </div>
        ...
        <div className="detail-thumbs">
          {imgs.slice(0, VISIBLE_THUMBS).map((src, i) => (
            <button
              key={i}
              type="button"
              className={`detail-thumb ${imgIdx === i ? 'active' : ''}`}
              onClick={() => setImgIdx(i)}
            >
              <OptimizedImage src={src} alt="" width={160} />
            </button>
          ))}
          {extraCount > 0 && (
            <button type="button" className="detail-thumb detail-thumb-more" onClick={() => setGalleryOpen(true)}>
              +{extraCount}
            </button>
          )}
        </div>
      ...
      <div className="detail-body">
        <div className="detail-price-block">
          <div>
            ...
            <div className="detail-rating">
              ...
              <svg viewBox="0 0 24 24" fill="var(--blue)" width="14" height="14">
        ...
        <div className="detail-location-row">
          <span className="detail-location">
            <svg viewBox="0 0 24 24" fill="none" stroke="var(--blue)" strokeWidth="2" width="16" height="16">
        ...
        <div className="detail-contact-box">
          ...
          >
            ...
            <div>
              <strong>Contacter sur WhatsApp</strong>
            ...
            <div className="detail-amenities">
              {p.amenities.map(a => (
                <div key={a} className="detail-amenity">
                  <span className="detail-amenity-icon">{AMENITY_ICONS[a] || '✓'}</span>
      ...
      {galleryOpen && (
        <ImageGalleryModal images={imgs} initialIndex={imgIdx} onClose={() => setGalleryOpen(false)} />
      )}
    </article>
  )
}
